...
import { ArrowRight, BookOpen, CheckCircle2, Clock3, Euro, FileUp, ShieldCheck, Wrench } from 'lucide-react'
...
const guides = [
  ['/guias/que-es-ecu-file-service','Qué es un ECU File Service','Cómo funciona el flujo ORI → solicitud → MOD para un taller.'],
  ['/guias/como-preparar-archivo-ori','Cómo preparar un archivo ORI','Identificación, lectura, diagnosis y datos que conviene enviar.'],
  ['/guias/obd-vs-bench-vs-boot','OBD vs Bench vs Boot','Diferencias entre métodos de lectura y qué información necesita el File Service.'],
  ['/guias/stage-1-vs-stage-2-vs-stage-3','Stage 1 vs Stage 2 vs Stage 3','Qué significa cada nivel y por qué el hardware real importa.'],
  ['/guias/errores-al-enviar-archivos-file-service','Errores al enviar archivos','Los fallos más frecuentes antes de crear una solicitud ECU.'],
] as const
...
export default function EcuFileServicePage() {
  ...
  return (
    <main className="ak-v5-bg min-h-screen text-white">
      ...
      <section className="border-y border-white/[.06] bg-white/[.018]"><div className="mx-auto max-w-[1280px] px-5 py-20 lg:px-8"><div className="flex items-center gap-2 text-[#67e8d1]"><BookOpen size={20}/><div className="ak-v5-kicker">Centro técnico AK Cloud</div></div><h2 className="ak-v5-title mt-4 text-4xl sm:text-5xl">Aprende antes de subir el archivo.</h2><p className="mt-5 max-w-3xl leading-7 text-white/45">Guías creadas para ayudar a talleres y preparadores a enviar mejores solicitudes, identificar correctamente la ECU y reducir errores de lectura, diagnosis y versiones.</p><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{guides.map(([href,title,text]) => <Link key={href} href={href} className="ak-v5-card group p-6 transition hover:border-white/20"><div className="text-lg font-black">{title}</div><p className="mt-3 text-sm leading-6 text-white/45">{text}</p><div className="mt-5 flex items-center gap-2 text-xs font-black text-[#ff425a]">Leer guía <ArrowRight size={14} className="transition group-hover:translate-x-1"/></div></Link>)}</div><Link href="/guias" className="mt-8 inline-flex items-center gap-2 font-black text-[#67e8d1]">Ver todas las guías <ArrowRight size={16}/></Link></div></section>
      <section className="border-t border-white/[.06]"><div className="mx-auto grid max-w-[1280px] gap-10 px-5 py-20 lg:grid-cols-2 lg:px-8"><div><div className="ak-v5-kicker">Compatibilidad profesional</div><h2 className="ak-v5-title mt-4 text-4xl">Trabaja con tu equipo habitual.</h2><p className="mt-5 leading-7 text-white/45">El servicio está dirigido a profesionales capaces de obtener y escribir correctamente el archivo de la ECU o TCU. La compatibilidad concreta depende del vehículo, ECU, protocolo y tipo de lectura.</p><p className="mt-5 leading-7 text-white/45">AK Cloud forma parte del ecosistema profesional de <a href="https://www.autokeysremapspro.es/" target="_blank" rel="noopener" className="font-bold text-[#ff425a]">Autokeys Remaps Pro</a>, laboratorio especializado en electrónica del automóvil, centralitas y programación.</p></div><div className="ak-v5-card p-7"><Clock3 className="text-[#ff425a]"/><h3 className="mt-5 text-2xl font-bold">¿Ya tienes el archivo original?</h3><p className="mt-3 leading-7 text-white/45">Crea tu cuenta profesional y utiliza AK Cloud como canal de trabajo: pedido, archivo, mensajes, versiones y entrega en el mismo workspace.</p><Link href="/register" className="ak-v5-button mt-7">Empezar con AK Cloud <ArrowRight size={17}/></Link></div></div></section>
    </main>
  )
}
