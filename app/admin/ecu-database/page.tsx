...
type FormState = { fabricante: string; ecu: string; familia: string; marcas: string; vehiculo: string; modelo: string; motor: string; potencia: string; anios: string; herramientas: string; servicios: string; patrones: string; tamanos: string; notas: string }
const emptyForm: FormState = { fabricante: 'Bosch', ecu: '', familia: '', marcas: '', vehiculo: '', modelo: '', motor: '', potencia: '', anios: '', herramientas: DEFAULT_TOOLS.join(', '), servicios: DEFAULT_SERVICES.join(', '), patrones: '', tamanos: '', notas: '' }
...
export default function EcuDatabasePage() {
  ...
  async function submit() {
    ...
    try {
      await createEcuRule({ fabricante: form.fabricante.trim() || null, ecu: form.ecu.trim(), familia: form.familia.trim(), marcas: csvToArray(form.marcas), vehiculo: form.vehiculo.trim() || null, modelo: form.modelo.trim() || null, motor: form.motor.trim() || null, potencia: form.potencia.trim() || null, anios: form.anios.trim() || null, herramientas: csvToArray(form.herramientas), servicios: csvToArray(form.servicios), patrones: csvToArray(form.patrones), tamanos: csvToNumberArray(form.tamanos), notas: form.notas.trim() || null, activo: true })
        ...
        <div className="grid gap-6 2xl:grid-cols-[480px_1fr]">
          <AKCard className="p-5 md:p-6">
            ...
            <div className="grid gap-3">
              ...
              <div className="grid gap-3 md:grid-cols-2"><Field label="Motor" value={form.motor} onChange={(v) => setField('motor', v)} placeholder="2.0 TDI" /><Field label="Potencia" value={form.potencia} onChange={(v) => setField('potencia', v)} placeholder="150 CV" /></div>
