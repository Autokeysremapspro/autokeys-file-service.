...
const statusStyles: Record<string, string> = {
  green: 'border-[#67e8d1]/25 bg-[#67e8d1]/10 text-[#67e8d1]',
  amber: 'border-amber-400/25 bg-amber-400/10 text-amber-300',
  slate: 'border-white/15 bg-white/[.06] text-white/55',
}
  ...
  return (
    <main className="ak-v5-bg min-h-screen overflow-hidden text-white">
      <header className="ak-v5-topbar sticky top-0 z-50">
        <div className="mx-auto flex max-w-[1480px] items-center justify-between px-5 py-4 lg:px-8">
          ...
          <div className="flex gap-2">
            ...
            <Link href="/register" className="ak-v5-button hidden !px-4 !py-2.5 text-xs sm:inline-flex"><UserPlus size={15} /> Solicitar cuenta</Link>
          </div>
        </div>
      </header>
      ...
      <section className="relative z-10 overflow-hidden">
        ...
        <div className="ak-v5-reveal">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            <Link href="/register" className="ak-v5-button"><UploadCloud size={18} /> Subir archivo</Link>
            ...
            <div className="flex items-center justify-between border-b border-white/[.07] px-4 py-3">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="grid h-8 w-8 place-items-center rounded-full border border-[#ff425a]/30 bg-[#ff425a]/10 text-[10px] font-black text-[#ff8095]">TP</div>
                </div>
              </div>
            </div>
            <div className="grid gap-3 p-3 sm:grid-cols-[170px_1fr]">
              <aside className="hidden rounded-2xl border border-white/[.07] bg-black/25 p-3 sm:block">
                {['Dashboard', 'Subir archivo', 'Mis pedidos', 'Historial', 'Servicios', 'Mensajes', 'Mi cuenta', 'Soporte'].map((x, i) => (
                  <div key={x} className={`mb-1.5 rounded-xl px-3 py-2.5 text-xs ${i === 0 ? 'bg-[#ff425a]/[.12] text-[#ff8095] font-bold' : 'text-white/35'}`}>{x}</div>
                ))}
                <div className="mt-4 border-t border-white/[.07] pt-3 text-xs font-bold text-[#ff425a]">Cerrar sesión</div>
              </aside>
              <div className="space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="ak-v5-card p-4">
                    ...
                    <div className="mt-3 grid place-items-center rounded-2xl border border-dashed border-white/15 bg-black/20 py-6 text-center">
                      <UploadCloud size={22} className="text-[#ff425a]" />
                  ...
                  <div className="ak-v5-card p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-bold text-[#ff425a]">Ver todos</span>
                    </div>
                ...
                <div className="ak-v5-card p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold text-[#ff425a]">Ver todos los servicios</span>
                  </div>
                  <div className="mt-3 grid grid-cols-4 gap-2 text-center">
                    <div className="rounded-xl border border-white/[.07] bg-black/20 p-2.5"><Gauge size={16} className="mx-auto text-[#ff8095]" /><div className="mt-1.5 text-[9px] font-bold text-white/55">Stage 1<br />Stage 2<br />Stage 3</div></div>
                    <div className="rounded-xl border border-white/[.07] bg-black/20 p-2.5"><Filter size={16} className="mx-auto text-[#ff8095]" /><div className="mt-1.5 text-[9px] font-bold text-white/55">DPF / EGR<br />AdBlue / Lambda</div></div>
                    <div className="rounded-xl border border-white/[.07] bg-black/20 p-2.5"><Cpu size={16} className="mx-auto text-[#ff8095]" /><div className="mt-1.5 text-[9px] font-bold text-white/55">Immo Off<br />soluciones ECU</div></div>
                    <div className="rounded-xl border border-white/[.07] bg-black/20 p-2.5"><LayoutGrid size={16} className="mx-auto text-[#ff8095]" /><div className="mt-1.5 text-[9px] font-bold text-white/55">Más servicios<br />personalizados</div></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="mx-auto grid max-w-[1480px] grid-cols-2 gap-4 px-5 py-8 md:grid-cols-4 lg:px-8">
          {highlights.map(({ icon: Icon, title, text }) => (
            <div key={title} className="flex items-start gap-3">
              <div className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-[#ff425a]/20 bg-[#ff425a]/10 text-[#ff8095]"><Icon size={19} /></div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
          <div className="ak-v5-card p-7 sm:p-8">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-bold text-[#ff425a]">Ver todo el historial</span>
            </div>
      ...
      <div className="fixed inset-x-3 bottom-3 z-50 rounded-2xl border border-white/10 bg-[#080a0f]/90 p-2 shadow-2xl backdrop-blur-xl sm:hidden">
        <Link href="/register" className="ak-v5-button w-full justify-center">Solicitar cuenta <ArrowRight size={17} /></Link>
      </div>
      ...
      <footer id="contacto" className="relative z-10 border-t border-white/[.07]">
        <div className="mx-auto grid max-w-[1480px] gap-10 px-5 py-14 lg:grid-cols-[1.3fr_.8fr_.8fr_.8fr_1fr] lg:px-8">
          ...
          <div>
            <div className="text-sm font-black">Autokeys <span className="text-[#ff425a]">Remaps Pro</span></div>
