...
const commercialLinks = [
  ['/en/stage-1-file-service', 'Stage 1 tuning files'],
  ['/en/ecu-file-service/tuning-files-online', 'ECU tuning files online'],
  ['/en/ecu-file-service/diesel-ecu-tuning-files', 'Diesel ECU tuning files'],
  ['/en/ecu-file-service/petrol-tuning-files', 'Petrol ECU tuning files'],
  ['/en/ecu-file-service/performance-tuning', 'Performance tuning files'],
  ['/en/ecu-file-service/custom-tuning', 'Custom ECU tuning files'],
  ['/en/ecu-file-service/workshops', 'ECU file service for workshops'],
  ['/en/ecu-file-service/tuners', 'ECU file service for tuners'],
  ['/en/ecu-file-service/edc17', 'Bosch EDC17 file service'],
  ['/en/ecu-file-service/md1', 'Bosch MD1 file service'],
  ['/en/ecu-file-service/mg1', 'Bosch MG1 file service'],
  ['/en/ecu-file-service/autotuner', 'AutoTuner file service'],
] as const
...
export default function EnglishEcuFileServicePage() {
  ...
  return <main className="ak-v5-bg min-h-screen text-white">
    ...
    <section className="border-t border-white/[.06] bg-white/[.018]"><div className="mx-auto max-w-[1180px] px-5 py-20 lg:px-8"><div className="ak-v5-kicker">ECU FILE SERVICE DIRECTORY</div><h2 className="ak-v5-title mt-4 text-4xl">Explore tuning files, ECU families and professional workflows.</h2><p className="mt-5 max-w-3xl leading-7 text-white/45">Use the most specific AK Cloud page for the type of ECU file request you handle. This internal directory connects our strongest commercial and technical file-service pages.</p><div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{commercialLinks.map(([href,label])=><Link key={href} href={href} className="ak-v5-card group p-6 transition hover:border-white/20"><h3 className="text-base font-black">{label}</h3><div className="mt-4 flex items-center gap-2 text-xs font-black text-[#ff425a]">View service <ArrowRight size={14} className="transition group-hover:translate-x-1"/></div></Link>)}</div></div></section>
  </main>
}
