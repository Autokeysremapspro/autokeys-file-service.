...
export default async function ToolPage({ params }: { params: Promise<{ tool: string }> }) {
  ...
  return (
    <main className="ak-v5-bg min-h-screen text-white">
      ...
      <section className="border-y border-white/[.06] bg-white/[.018]"><div className="mx-auto max-w-[1180px] px-5 py-20 lg:px-8"><div className="ak-v5-kicker">Compatibilidad</div><h2 className="ak-v5-title mt-4 text-4xl">La herramienta no determina por sí sola la solución disponible.</h2><p className="mt-6 max-w-3xl leading-7 text-white/45">Antes de solicitar un archivo, identifica correctamente vehículo, motor, ECU, hardware, software y método de lectura. La disponibilidad depende de la referencia concreta y del contenido leído.</p><div className="mt-7 flex flex-wrap gap-4"><Link href={`/guias/file-service/${tool}`} className="font-bold text-[#67e8d1]">Guía de trabajo con {item.name} →</Link><Link href="/stage-1-file-service" className="font-bold text-[#ff425a]">Stage 1 File Service →</Link><Link href="/ecu-file-service/edc17" className="font-bold text-white/60">Bosch EDC17 →</Link><Link href="/ecu-file-service/md1" className="font-bold text-white/60">Bosch MD1 →</Link><Link href="/ecu-file-service/mg1" className="font-bold text-white/60">Bosch MG1 →</Link></div></div></section>
