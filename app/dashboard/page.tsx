  ...
  return <AppShell><div className="space-y-6">
    ...
    <section className="grid gap-5 2xl:grid-cols-[1.3fr_.85fr_.85fr]">
      ...
      <div className="ak5-card relative overflow-hidden rounded-[26px] p-5 sm:p-6">
        <div className="pointer-events-none absolute -left-10 -top-14 h-52 w-52 rounded-full" style={{background:'radial-gradient(circle,rgba(34,211,238,.12),transparent 70%)'}}/>
        <div className="relative flex items-center justify-between"><div><div className="ak5-kicker text-cyan-300">Distribución</div><h3 className="mt-1 text-xl font-black">Servicios pedidos</h3></div></div>
        {servicios.length===0?<div className="relative mt-8 py-8 text-center text-sm text-white/30">Sin datos de servicios todavía.</div>:<>
          ...
          <div className="relative mt-6 space-y-2">{servicios.map(s=><div key={s.label} className="flex items-center justify-between text-xs"><span className="flex items-center gap-2 font-bold text-white/70"><i className="h-2.5 w-2.5 rounded-full" style={{background:s.color,boxShadow:`0 0 8px ${s.color}`}}/>{s.label}</span><span className="text-white/35">{s.pct}% ({s.value})</span></div>)}</div>
        </>}
      </div>

      <div className="ak5-card relative overflow-hidden rounded-[26px] p-5 sm:p-6">
        <div className="pointer-events-none absolute -right-10 -bottom-14 h-52 w-52 rounded-full" style={{background:'radial-gradient(circle,rgba(52,211,153,.1),transparent 70%)'}}/>
        <div className="relative flex items-center justify-between"><div><div className="ak5-kicker text-emerald-300">Pipeline</div><h3 className="mt-1 text-xl font-black">Timeline global</h3></div></div>
        <div className="relative mt-6 space-y-4">{pipeline.map((s,i)=>{const Icon=s.icon;const colors=['border-red-400/20 bg-red-400/10 text-red-300','border-amber-400/20 bg-amber-400/10 text-amber-300','border-cyan-400/20 bg-cyan-400/10 text-cyan-300','border-emerald-400/20 bg-emerald-400/10 text-emerald-300'];return <div key={s.label} className="flex items-center gap-3"><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl border ${colors[i]}`}><Icon size={16}/></span><div className="min-w-0 flex-1"><div className="text-xs font-bold text-white/70">{s.label}</div></div><span className="text-lg font-black">{s.value}</span></div>})}</div>
      </div>
    </section>
