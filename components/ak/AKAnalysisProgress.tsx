...
const steps = [
  ...
  { label: 'Verificando checksum', icon: ShieldCheck },
  { label: 'Comprobando servicios compatibles', icon: Sparkles },
]
    ...
    <div className="rounded-[2rem] border border-white/10 bg-black/35 p-5 shadow-[0_30px_90px_rgba(0,0,0,.35)]">
      <div className="mb-4 flex items-center justify-between gap-4">
        <div>
          ...
          {!done && <p className="mt-1 text-xs text-white/40">Las comprobaciones se confirman únicamente al finalizar el análisis.</p>}
        </div>
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${done ? 'bg-[var(--ak-green)]/15 text-[var(--ak-green)]' : 'bg-[var(--ak-red)]/15 text-[var(--ak-glow)]'}`}>
        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.label} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.035] p-3">
              <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${done ? 'bg-[var(--ak-green)]/12 text-[var(--ak-green)]' : 'bg-white/5 text-white/45'}`}>
                {done ? <CheckCircle2 size={17} /> : <Icon size={17} />}
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-3 text-sm font-bold">
                  <span className="truncate">{step.label}</span>
                  <span className="shrink-0 text-xs text-white/40">{done ? 'OK' : 'En curso'}</span>
                </div>
                <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/5">
                  <div className={`h-full rounded-full ${done ? 'w-full bg-[var(--ak-green)]' : 'w-1/2 animate-pulse bg-[var(--ak-glow)]'}`} />
                </div>
              </div>
            </div>
          )
